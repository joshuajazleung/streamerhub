<template>
    <div>
        ...
        <p>StreamerHub is a Peer-to-Peer file streaming service. It allows users to directly stream files to other users.</p>
        <p>The more people are downloading the same file, the faster the streaming speed will be. So, share the link to other people as much as you can if you want fast streaming speed.</p>
        <fqa></fqa>
        <div v-if="loading" class="mt-5">
            <p
                class="text-center text-2xl text-white"
            >File is being loaded from the server. Please wait.</p>
            <v-progress-linear :indeterminate="true"></v-progress-linear>
        </div>
        <div class="mt-5" v-if="isSeeding">
            <h2>Seeding</h2>
            <p>You are seeding the files. Open this browser window as long as possible until the "ratio" reach 1.00 or higher. Otherwise other people won't be able to download.</p>
            <div v-for="(perSeed, i) in seedInfo" :key="i">
                ...
                <p>File Size: {{ perSeed.info.length | humanBytes }} Peers: {{perSeed.info.peers}}, UploadSpeed: {{ perSeed.info.uploadSpeed }}, Ratio: {{ perSeed.info.ratio }}</p>
                <ul>
        ...
        <div v-if="addedFiles" class="mt-5">
            ...
            <p
                class="mb-5"
            >File size: {{ torrentInfo.length | humanBytes }}, Progress: {{ torrentInfo.progress }}, Peers: {{ torrentInfo.peers }}, Download Speed: {{ torrentInfo.downloadSpeed }}, Upload Speed: {{ torrentInfo.uploadSpeed }}, ETA: {{ torrentInfo.remaining }}, Ratio: {{ torrentInfo.ratio }}</p>
            <h2>File List</h2>
            <v-btn
        ...
        <p>Download from a magnet link or info hash
            <v-form>
